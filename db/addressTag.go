...
// AddAddrTag 添加地址标签
func AddAddrTag(db *sql.DB, tag entities.AddrTag) error {
...
func UpdateAddrTag(db *sql.DB, tag entities.AddrTag) error {
...
func GetAddrTags(db *sql.DB) ([]entities.AddrTag, error) {
	rows, err := db.Query("SELECT * FROM T_ADDR_TAG")
	if err != nil {
		return nil, err
	}
	var tags []entities.AddrTag
	for rows.Next() {
		var tag entities.AddrTag
