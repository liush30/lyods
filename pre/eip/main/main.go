...
func main() {
	var val interface{} = 36130
	v, i := interfaceToData(val, "uint32")
	fmt.Println(i)
	fmt.Println(v)
	...
	//		LastRequestTime: time.Now(),
	//	}
	//	transList, block, err := ethClient.GetTxListOnEth(&cbClient, "0x7Db418b5D567A4e0E8c59Ad71BE1FcE48f3E6107", "0")
	//	if err != nil {
	//		log.Println("出错啦:", err.Error())
	//	}
	//	fmt.Println("block = ", block)
	//	fmt.Println("transList length = ", len(transList))
...
func interfaceToData(dataItem interface{}, dataType string) (string, bool) {
	...
	case "uint8", "uint16", "uint32", "uint64", "int8", "int16", "int32", "int64":
		if intValue, intOk := dataItem.(int); intOk {
